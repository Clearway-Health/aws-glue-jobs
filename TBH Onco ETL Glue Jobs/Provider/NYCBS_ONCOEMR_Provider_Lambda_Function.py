...
def handle_job_result(status, job_status, s3_key, sns_client, sns_topic_arn):
    sns_subject = f'Glue Job Completed - NYCBS_ONCOEMR_Provider_Job - Status: {status}'
    sns_message = f'The Glue job NYCBS_ONCOEMR_Provider_Job has {status} for file {s3_key}.'
...
def handle_glue_error(e, sns_client, sns_topic_arn):
    ...
    sns_subject = 'Glue Job Error - NYCBS_ONCOEMR_Provider_Job'
    sns_message = f"""
    Error occurred while processing Glue job NYCBS_ONCOEMR_Provider_Job.
...
def handle_error(e, sns_client, sns_topic_arn, error_type, status_code):
    logger.error(f"{error_type}: {e}")
    sns_subject = 'Lambda Function Error - NYCBS_ONCOEMR_Provider_Job'
    sns_message = f"""
    Error occurred in Lambda function for NYCBS_ONCOEMR_Provider_Job.
    Error type: {error_type}
    Error message: {str(e)}
    """
    publish_sns_message(sns_client, sns_topic_arn, sns_subject, sns_message)
