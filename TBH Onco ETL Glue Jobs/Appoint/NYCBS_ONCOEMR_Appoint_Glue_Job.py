...
transformed_file_key = f'{temp_folder}nycbs_oncoemr_transformed_file.csv'
...
df['Appointment Date'] = pd.to_datetime(df['Appointment Date']).dt.strftime('%m-%d-%Y')
...
df = df[['Patient MRN','Provider Last Name','Provider First Name','Department ID','Department Name','Appointment Date','Appointment Time','Appointment Type','Appointment Description','Appointment Status','File Name','Load Timestamp']]
...
truncate_table(jdbc_url, "dbo.NYCBS_ONCOEMR_Appoint", username, password)
...
try:
    glueContext.write_dynamic_frame.from_jdbc_conf(
        ...
        connection_options={
            "dbtable": "dbo.NYCBS_ONCOEMR_Appoint",
            "database": database,
            "user": username,
            "password": password
        },
    ...
    try:
        spark_df.write \
            .format("jdbc") \
            .option("url", jdbc_url) \
            .option("dbtable", "dbo.NYCBS_ONCOEMR_Appoint") \
