...
#df['Patient DOB'] = pd.to_datetime(df['Patient DOB']).dt.strftime('%m-%d-%Y')
df['Patient DOB'] = pd.to_datetime(df['Patient DOB'], errors='coerce').dt.strftime('%m-%d-%Y')

df['Patient Phone Number'] = df['Patient Phone Number'].apply(lambda x: re.sub(r'\D', '', str(x)))
...
truncate_table(jdbc_url, "dbo.NYCBS_ONCOEMR_PATDemo", username, password)
...
try:
    glueContext.write_dynamic_frame.from_jdbc_conf(
        ...
        connection_options={
            "dbtable": "dbo.NYCBS_ONCOEMR_PATDemo",
            "database": database,
            "user": username,
            "password": password
        },
    ...
    try:
        spark_df.write \
            .format("jdbc") \
            .option("url", jdbc_url) \
            .option("dbtable", "dbo.NYCBS_ONCOEMR_PATDemo") \
