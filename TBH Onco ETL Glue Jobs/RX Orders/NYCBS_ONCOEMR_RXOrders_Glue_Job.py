...
df = df.rename(columns={'pharmacyid':'Pharmacy ID'})

df['Date Sent'] = pd.to_datetime(df['Date Sent']).dt.strftime('%m-%d-%Y')
...
truncate_table(jdbc_url, "dbo.TBH_Onco_RXOrder", username, password)
...
try:
    ...
    try:
        spark_df.write \
            .format("jdbc") \
            .option("url", jdbc_url) \
            .option("dbtable", "dbo.TBH_Onco_RXOrder") \
