...
def lambda_handler(event, context):
    ...
    sns_topic_arn = 'arn:aws:sns:us-east-1:030063318327:Onco_Clearway_Provider_Notifications'
    ...
    try:
        ...
        if s3_key.startswith('DecryptedFiles/TBH-CWH-TFS/NYCBS_ONCOEMR_Provider'):
            glue_client = boto3.client('glue', region_name='us-east-1')
            try:
                response = glue_client.start_job_run(
                    JobName='NYCBS_ONCOEMR_Provider_Job',
                    Arguments={
                ...
                while True:
                    job_status = glue_client.get_job_run(JobName='NYCBS_ONCOEMR_Provider_Job', RunId=job_run_id)
                ...
                if status == 'SUCCEEDED':
                    logger.info(f'Glue job NYCBS_ONCOEMR_Provider_Job completed successfully for file {s3_key}.')
                    sns_message = f'Glue job NYCBS_ONCOEMR_Provider_Job completed successfully for file {s3_key}.'
                    sns_client.publish(
                    ...
                    logger.error(f'Glue job NYCBS_ONCOEMR_Provider_Job failed for file {s3_key}. Status: {status}. Error: {error_message}')
                    sns_message = f'Glue job NYCBS_ONCOEMR_Provider_Job failed for file {s3_key}. Status: {status}. Error: {error_message}'
                    sns_client.publish(
                        TopicArn=sns_topic_arn,
                        Subject='Glue Job Failed for NYCBS_ONCOEMR_Provider*',
                        Message=sns_message
                    )
                return {
                    'statusCode': 200,
        ...
        sns_client.publish(
            TopicArn=sns_topic_arn,
            Subject='Lambda Function Error for NYCBS_ONCOEMR_Provider',
            Message=sns_message
        )
        return {
            'statusCode': 400,
        ...
        sns_client.publish(
            TopicArn=sns_topic_arn,
            Subject='Lambda Function Error NYCBS_ONCOEMR_Provider',
            Message=sns_message
        )
        return {
            'statusCode': 500,
